   ...
   boolean typeConnectable(int outpoint, int inpoint) {
    if (inpoint == NData.NDATA || outpoint == NData.NDATA) {
      return true;
    }

    if ((outpoint == inpoint) || ((outpoint == NData.NINT) && (inpoint == NData.NDOUBLE)))
      return true;
    else
      return false;
  }
   ...
   NData clone(NData data, int type) {
    switch (type) {
    case NData.NDOUBLE:
...
class NData extends NObject {
  ...
  public NData clone() {
     switch (TYPE) {
    case NData.NDOUBLE:
      return new NDouble(getDoubleData());
    case NData.NINT:
      return new NInt(getIntData());
    case NData.NSTRING:
      return new NString(getStringData());
    case NData.NBOOLEAN:
      return new NBoolean(getBooleanData());
    }
    return null;
  }
...
class NImage extends NData {
  private ArrayList<NVector> data = new ArrayList<NVector>();
