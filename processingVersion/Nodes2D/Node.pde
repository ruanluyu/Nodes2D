...
class Node extends NObject {
  protected ArrayList<NodePoint> inPointList = new ArrayList<NodePoint>();
  protected ArrayList<NodePoint> outPointList = new ArrayList<NodePoint>();
  ...
  void clone(Node originN, Node clonedN) {
  ...
  public void addStreamToOutpoint(int id, NData data) {
    ...
    for (int i = 0; i < cur.getNumOfLines(); i++) {
      cur.addStream(new NStream(data.clone(), cur, cur.getLine(i)));
    }
  }
